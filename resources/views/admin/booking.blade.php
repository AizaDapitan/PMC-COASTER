        ...
        <div class="tab-content">
          <div class="tab-pane row fade in active" id="tab-1">                       
            <div class="col-md-12 col-sm-12">
              <table><tr><td>
                  @if($print)
                    <button type="button" class="btn green" onclick="window.open(document.URL + '/satprint','displayWindow','toolbar=no,scrollbars=yes,width=1000')";><i class="fa fa-print"></i> Print</button>
                  @else
                    <button disabled type="button" class="btn green" onclick="window.open('/admin/booking/satprint','displayWindow','toolbar=no,scrollbars=yes,width=1000')";><i class="fa fa-print"></i> Print</button>
                  @endif
                  </td></tr></table>
               <table class="table">
                <thead>
                        ...
                        @if(in_array($i, $bookingSeats)) 
                          ...
                          <td>
                            <form method ="POST" action="/admin/booking/{{ $bookings[array_search($i, $bookingSeats)]->id }}">
                              ...
                              @else
                                <button disabled class="btn red btn-sm" id="show_confirm" type="submit">Cancel Booking</button>
                              @endif
          ...
          <div class="tab-pane row fade" id="tab-2">
            <div class="col-md-12 col-sm-12">
              ...
              <table><tr><td>
                @if($print)
                  <button type="button" class="btn green" onclick="window.open( document.URL + '/mondayprint','displayWindow','toolbar=no,scrollbars=yes,width=1000')";><i class="fa fa-print"></i> Print</button>
                @else
                  <button disabled type="button" class="btn green" onclick="window.open('/admin/booking/mondayprint','displayWindow','toolbar=no,scrollbars=yes,width=1000')";><i class="fa fa-print"></i> Print</button>
              @endif
                  ...
                  @for ($i = 1; $i <= config('constant.totalNumberOfSeat') ; $i++)
                    <tr style='background-color:{{ $i % 2 ? "#FFFFFF" : "#F8F8AC" }}'>
                        ...
                        @if(in_array($i, $bookingSeatsMonday)) 
                          ...
                          <td>
                            <form method ="POST" action="{{ env('APP_URL')}}/admin/booking/{{ $bookingsMonday[array_search($i, $bookingSeatsMonday)]->id }}">
                              @method('DELETE')
                              @csrf
